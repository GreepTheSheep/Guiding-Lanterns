...
function giveawayCommands(message, client, prefix, functiondate, functiontime, getlogchannel, lang){
    if (message.content.toLowerCase().startsWith(prefix + 'giveaway')){
        if (!message.member.roles.cache.some(role => role.name.toLowerCase().includes('giveaway')) || message.member.hasPermission('MANAGE_MESSAGES')) return message.channel.send(lang.giveaway_noPerms)
